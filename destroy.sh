...
# (control-plane + worker EC2 nodes / VPC / subnet / security groups, etc.)
...
#
# WARNING: This terminates every EC2 node created by Terraform. Any data on the
# nodes (including the Kubernetes local-path PostgreSQL data) is lost.
...
if [ "${1:-}" != "--yes" ]; then
  ...
  echo "  - EC2 nodes (control-plane + workers, all terminated)"
  echo "  - VPC / subnet / internet gateway / route table"
  echo "  - security groups and IAM role/profile"
  echo
  echo "Any data on the nodes (incl. Kubernetes PostgreSQL) is LOST."
  echo "============================================================"
