...
variable "instance_type" {
  description = "EC2 instance type for every DevShop node. Defaults to t3.micro so the whole fleet stays within the AWS Free Tier (750 instance-hours/month). Run only when needed and stop the box when idle, or bump the type if a node gets memory-pressured."
  type        = string
  default     = "t3.micro"

  validation {
    condition     = can(regex("^[a-z][0-9][a-z]?(\\.[a-z0-9]+)?$", var.instance_type))
    error_message = "instance_type must be a valid EC2 type, e.g. t3.micro."
  }
}

variable "control_plane_count" {
  description = "Number of Kubernetes control-plane nodes. Keep at 1 — this is a single-master learning cluster, not HA."
  type        = number
  default     = 1
...
variable "worker_count" {
  description = "Number of worker nodes. Worker 1 hosts the application (frontends, backend, PostgreSQL); worker 2 hosts the monitoring stack. With 1 worker everything is co-located on it."
  type        = number
  default     = 2
